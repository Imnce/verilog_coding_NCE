...
For repeated or multi-instance top-level interfaces, use symmetric suffixes on every instance. Do not leave the first instance bare while later instances use `_1`. Use names such as `RXP_0` / `RXP_1`, `TXP_0` / `TXP_1`, `PMA_INIT_0` / `PMA_INIT_1`, and `HARD_ERR_0` / `HARD_ERR_1`. Apply this to Aurora, GT, AXIS, AXI, status, reset, and other repeated interface ports so XDC constraints and waveform debug stay unambiguous.
...
Synthesizable RTL files use one module per file. The file name should match the module name, except vendor IP wrappers generated by the FPGA toolchain. Testbench files may include local simulation models or stub modules when that keeps the test self-contained.

For open-drain interfaces such as I2C, make the tristate structure explicit: split each bidirectional pin into internal `in`, `out`, and `oe` signals before assigning the `inout` port. For I2C, `out` is normally fixed at `1'b0`, and `oe=1` means drive low while `oe=0` means release to high-Z. This makes SDA/SCL output intent visible in waveforms and code review.
...
When modifying: preserve ALL `Revision` history. Append new entry with version, description, date (y/m/d/h/m = 年/月/日/时/分):
```verilog
// Revision 0.02 - add burst mode support, 2026/05/17/14/30
// Revision 0.03 - fix spi timing, 2026/05/18/09/15
```
...
Named port connections, one per line. Parameters via `#()`. Align port names, opening parentheses, connection expressions, closing parentheses, and commas into neat columns within each instance. For modules with many ports, group instance connections by interface or signal type, not by input/output direction. Keep related signals together: AXI channel with same AXI bus, SPI pins together, UART pins together, JTAG pins together, clock/reset together, and module-specific user/backend command signals together. Add section dividers inside the instance before each group.

AXI instance ports must be grouped by bus, then by AXI channel. For each AXI bus, use this channel order:
1. Write address channel: `AWADDR`, `AWBURST`, `AWCACHE`, `AWID`, `AWLEN`, `AWLOCK`, `AWPROT`, `AWQOS`, `AWREGION`, `AWSIZE`, `AWUSER`, `AWVALID`, `AWREADY`
2. Write data channel: `WDATA`, `WLAST`, `WSTRB`, `WUSER`, `WVALID`, `WREADY`
3. Write response channel: `BREADY`, `BID`, `BRESP`, `BUSER`, `BVALID`
4. Read address channel: `ARADDR`, `ARBURST`, `ARCACHE`, `ARID`, `ARLEN`, `ARLOCK`, `ARPROT`, `ARQOS`, `ARREGION`, `ARSIZE`, `ARUSER`, `ARVALID`, `ARREADY`
5. Read data channel: `RREADY`, `RDATA`, `RID`, `RLAST`, `RRESP`, `RUSER`, `RVALID`

For multiple AXI buses such as AXI0/AXI1/AXI2, repeat the same five-channel order for each bus before moving to non-AXI pins. Do not reorder vendor IP internals only for readability; apply this rule to wrapper-level declarations, assignments, and instance port maps.
...
Large instance example:
```verilog
axi_lite_mram u0_axi_lite_mram (
    //----------------------------------------------------------------
    // AXI clock/reset signal
    //----------------------------------------------------------------
    .S_AXI_ACLK    (i_clk        ),
    .S_AXI_ARESETN (i_axi_aresetn),
    //----------------------------------------------------------------
    // AXI write address channel
    //----------------------------------------------------------------
    .i_axi_awaddr  (w_axi_awaddr ),
    .i_axi_awprot  (w_axi_awprot ),
    .i_axi_awvalid (w_axi_awvalid),
    .o_axi_awready (w_axi_awready),
    //----------------------------------------------------------------
    // AXI write data channel
    //----------------------------------------------------------------
    .i_axi_wdata   (w_axi_wdata  ),
    .i_axi_wstrb   (w_axi_wstrb  ),
    .i_axi_wvalid  (w_axi_wvalid ),
    .o_axi_wready  (w_axi_wready )
);
```
...
- Do not use Verilog `function` in synthesizable project RTL. Use explicit `always @(*)` decode logic, local modules, or generated tables so the datapath is visible in waveform review.
